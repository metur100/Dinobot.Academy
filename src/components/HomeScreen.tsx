...
import { MISSIONS, DINOBOTS, Mission } from '../data/gameData';
...
const HomeScreen: React.FC<Props> = ({ missionStars, onSelect }) => {
  ...
  const r = total / max;
  const upgrade = r >= 0.8 ? { label: t.upgrades.ultra, color: '#ffe66d' }
    : r >= 0.5 ? { label: t.upgrades.power, color: '#4ecdc4' }
    : r >= 0.2 ? { label: t.upgrades.battle, color: '#ff6b35' }
    ...
    <div style={{ minHeight:'100vh', background:'var(--bg)', padding:'20px', overflowY:'auto' }}>
      ...
      <div style={{ textAlign:'center', marginBottom:28 }}>
        <div style={{ display:'flex', justifyContent:'center', gap:6, marginBottom:14, flexWrap:'wrap' }}>
          {[
            {src:'/images/optimus-face.jpg', alt:'Optimus'},
            {src:'/images/Grimlock.webp', alt:'Grimlock'},
            {src:'/images/trex.jpg', alt:'T-Rex'},
            {src:'/images/optimus-pose.jpg', alt:'Optimus'},
          ].map((it,i)=>(
            <img key={i} src={it.src} alt={it.alt} style={{ width:62, height:62, borderRadius:18, objectFit:'cover', border:'3px solid #2a3a4a', animation:`float ${2.2+i*0.35}s ease-in-out infinite`, filter:'drop-shadow(0 0 10px rgba(0,0,0,0.45))' }} />
          ))}
        </div>
        <h1 style={{
        ...
        <div style={{
          ...
          padding:'9px 22px', background:'var(--card)', borderRadius:50,
          border:`2px solid ${upgrade.color}`, boxShadow:`0 0 18px ${upgrade.color}44`,
        }}>
          <span style={{ fontSize:'1.1rem' }}>⭐ {total} / {max}</span>
          <span style={{ color:upgrade.color, fontFamily:'Fredoka One,cursive', fontSize:'0.9rem' }}>{upgrade.label}</span>
        </div>
      </div>

      {/* Mission grid */}
      <div style={{ display:'grid', gridTemplateColumns:'repeat(auto-fill,minmax(250px,1fr))', gap:14, maxWidth:900, margin:'0 auto' }}>
        {MISSIONS.map(m => {
          ...
          return (
            <button key={m.id} onClick={() => onSelect(m)} style={{
              ...
              borderRadius:18, padding:'20px', textAlign:'left',
            ...
            >
              ...
              <div style={{ display:'flex', alignItems:'center', gap:12, marginBottom:12 }}>
                <div style={{ width:58, height:58, borderRadius:16, background:m.color+'22', border:`3px solid ${m.color}`, display:'flex', alignItems:'center', justifyContent:'center', fontSize:'1.8rem', flexShrink:0, overflow:'hidden' }}>
                  {m.id==='runner' ? (
                    <img src='/images/optimus-pose.jpg' alt='runner' style={{ width:'100%', height:'100%', objectFit:'cover' }} />
                  ) : m.id==='photoPuzzle' ? (
                    <img src='/images/Grimlock.webp' alt='puzzle' style={{ width:'100%', height:'100%', objectFit:'cover' }} />
                  ) : (
                    <span>{m.icon}</span>
                  )}
                </div>
      ...
      <div style={{ textAlign:'center', marginTop:36, color:'#2a3a4a', fontSize:'0.8rem' }}><p>{t.dinoFact}</p></div>
    </div>
  );
};
