...
interface Q { a: number; b: number; op: '+' | '-'; ans: number; }
...
const NumbersGame: React.FC<Props> = ({ onComplete, onBack }) => {
  ...
  const pick = useCallback((c: number) => {
    ...
    <GameShell current={answered + 1} total={TOTAL} score={score} onBack={onBack}>
      ...
      <div className="card" style={{ width: '100%', maxWidth: 560, textAlign: 'center' }}>
        ...
        <div style={{ display: 'flex', flexWrap: 'wrap', justifyContent: 'center', gap: 2, minHeight: 44, marginBottom: 12 }}>
          {Array.from({ length: q.a }, (_, i) => <img key={i} src="/images/optimus-face.jpg" alt="unit" style={{ width: 34, height: 34, borderRadius: 10, objectFit:'cover', marginRight: 6 }} />)}
          <span style={{ fontSize: '1.4rem', margin: '0 6px' }}>{q.op === '+' ? '➕' : '➖'}</span>
          {Array.from({ length: q.b }, (_, i) => <img key={i} src="/images/optimus-face.jpg" alt="unit" style={{ width: 34, height: 34, borderRadius: 10, objectFit:'cover', marginRight: 6, opacity: q.op === '-' ? 0.3 : 1 }} />)}
        </div>
        <div style={{ fontSize: '2.8rem', fontFamily: 'Fredoka One,cursive', background: '#1e2d3d', borderRadius: 16, padding: '14px 20px', marginBottom: 22, letterSpacing: 4 }}>
          {q.a} {q.op} {q.b} = ?
        </div>
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10 }}>
          {choices.map(c => {
            ...
            return (
              <button key={c} onClick={() => pick(c)} style={{ padding: '16px', borderRadius: 14, fontSize: '1.9rem', fontFamily: 'Fredoka One,cursive', background: bg, color: '#f0f4f8', border: `3px solid ${border}`, transition: 'all 0.2s', cursor: selected !== null ? 'default' : 'pointer' }}>
                {c}{isSel && (isOk ? ' ✅' : ' ❌')}
              </button>
            );
          })}
